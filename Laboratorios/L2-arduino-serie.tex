...
\begin{itemize}
	\item Definir un protocolo de comunicación, con las acciones a seguir para cada dato recibido. Se deberán incluir al menos diez acciones, que incluyan todos los siguientes tipos de acciones:
	\begin{enumerate}
		\item Encender un led.
		\item Encender un led parcialmente.
		\item Apagar un led.
		\item Establecer una combinación determinada.
	\end{enumerate}
	...
	\item Leer el estado de un interruptor y retornar los LEDs a un estado inicial en caso de estar presionado.
\end{itemize}
...
\begin{enumerate}[resume]
	...
	\item Se deberá definir el protocolo de comunicación a manera de una lista de caracteres que se podrán ingresar y el efecto que dichos caracteres tendrán en el sistema.
\end{enumerate}
...
\section{Prueba corta}

Después de realizar la práctica, cada estudiante realizará una prueba corta. Para dicha prueba, los estudiantes deberán estudiar los siguientes temas:

\begin{enumerate}
	\item Estructuras de control \texttt{if} y \texttt{switch}.
	\item Funciones de la librería \texttt{Serial} (\texttt{available()}, \texttt{print()}, \texttt{println()} y \texttt{read()}).
	\item Funciones para salidas analógicas en Arduino (\texttt{analogWrite()}).
\end{enumerate}
